...
// would the default state be a query to return the list of all the current scripts???
// so the state for this reducer would have to do with the scripts in the database and subsequently adding, creating, editting, deleting them??
// const sql = require('mssql')

// async () => {
//     try {
//         await sql.connect('mssql://OCCNT\cbass:Elpsycongroo9(@localhost/Epic Script Testing')
//         const result = await sql.query`select * from ScriptInfo`
//         console.dir(result)
//     } catch (err) {
//         // ... error checks
//     }
...
console.log(state);
    switch (action.type) {
        ...
        case 'VIEW_SCRIPT':
            return state.filter(({ ScriptID }) => ScriptID === action.ScriptID);  // need to return an array or soemthing of all the current scripts and put in in a table or list
